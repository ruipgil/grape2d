Grape2D.utils.TopDownBVHTreeDebugger = {
	renderNode: function(node, renderer, camera) {
		...
		if (node.getLeft()) {
			...
			Grape2D.utils.TopDownBVHTreeDebugger.renderNode(node.getLeft(), renderer, camera);
			renderer.setStrokeColor("rgba(0,0,0,1)");
		}
		if (node.getRight()) {
			renderer.setStrokeColor("rgba(0,0,255,0.6)");
			Grape2D.utils.TopDownBVHTreeDebugger.renderNode(node.getRight(), renderer, camera);
			renderer.setStrokeColor("rgba(0,0,0,1)");
		}
	},
	render: function(tree, renderer, camera) {
		if (tree.getRootNode()) {
			Grape2D.utils.TopDownBVHTreeDebugger.renderNode(tree.getRootNode(), renderer, camera);
		}
	}
};