...
Grape2D.Vector.createFromPoints=function(a,b){return new Grape2D.Vector(b.x-a.x,b.y-a.y)};Grape2D.Vector.createFromAngle=function(a,b){return new Grape2D.Vector(b*Grape2D.Math.cos(a),b*Grape2D.Math.sin(a))};Grape2D.Matrix=function(a,b,c,d,e,f,g,k,h){this.v=[];void 0!==a?this.v=[a,b,c,d,e,f,g,k,h]:this.identity()};
...
Grape2D.GenericCollisionChecker.prototype.circleVsPolygon=function(a,b){for(var c=b.getComputedVertexList(),d=a.getPosition(),e=Grape2D.Math.sq(a.getRadius()),f=0;f<c.length;f++)if(c[f].sqDistanceTo(d)>=e)return!0;return!1};Grape2D.GenericCollisionChecker.prototype.polygonVsPolygon=function(a,b){return!1};
...
Grape2D.SATCollisionChecker.prototype.computeIntervals=function(a,b){for(var c=[],d=Infinity,e=-Infinity,f,g,k=0;k<b.length;k++){for(var d=Infinity,e=-Infinity,h=0;h<a.length;h++)f=a[h].dot(b[k]),g=a[(h+1)%a.length].dot(b[k]),f>e&&(e=f),f<d&&(d=f),g>e&&(e=g),g<d&&(d=f);c.push({min:d,max:e})}return c};
Grape2D.SATCollisionChecker.prototype.aabbVsRay=function(a,b,c,d){d=d.rightNormal();for(var e=Infinity,f=-Infinity,g,k=Grape2D.SATCollisionChecker.aabbToVertexList(a),h=0;h<k.length;h++)g=k[h].dot(d),g>f?f=g:g<e&&(e=g);g=b.dot(d);return e<=g&&g<=f?0<=Grape2D.Math.overlaps({min:b.getX(),max:c.getX()},{min:a.getMinX(),max:a.getMaxX()})&&0<=Grape2D.Math.overlaps({min:b.getY(),max:c.getY()},{min:a.getMinY(),max:a.getMaxY()}):!1};
...
Grape2D.SATCollisionChecker.SHARED_AABB_TO_VERTEX_LIST[3].setX(a.getMaxX());Grape2D.SATCollisionChecker.SHARED_AABB_TO_VERTEX_LIST[3].setY(a.getMaxY());return Grape2D.SATCollisionChecker.SHARED_AABB_TO_VERTEX_LIST};Grape2D.SATCollisionChecker.prototype.circleVsRay=function(a,b,c,d){var e;e=a.getPosition();var f=a.getRadius();a={min:0,max:0};e=e.dot(d);e={min:e-f,max:e+f};b.dot(d);b.getX();c.getX();a.min=b.dot(d);a.max=c.dot(d);return 0<=Grape2D.Math.overlaps(a,e)};
Grape2D.SATCollisionChecker.prototype.polygonVsRay=function(a,b,c,d){var e=d.rightNormal(),f=a.getComputedVertexList();a={min:0,max:0};var g;g=this.computeIntervals(f,[e])[0];e=b.dot(e);return g.min<=e&&e<=g.max?(g=this.computeIntervals(f,[d])[0],b=b.dot(d),c=c.dot(d),b>c?(a.max=b,a.min=c):(a.max=c,a.min=b),0<=Grape2D.Math.overlaps(g,a)):!1};Grape2D.SATCollisionChecker.aabbAxis=[new Grape2D.Vector(1,0),new Grape2D.Vector(0,1)];Grape2D.CollisionDispatcher={aabbVsAabb:function(a,b,c){return a.aabbVsAabb(b,c)},aabbVsCircle:function(a,b,c){return a.aabbVsCircle(b,c)},aabbVsPolygon:function(a,b,c){return a.aabbVsPolygon(b,c)},aabbVsPoint:function(a,b,c){return a.aabbVsPoint(b,c)},circleVsAabb:function(a,b,c){return a.circleVsAabb(b,c)},circleVsCircle:function(a,b,c){return a.circleVsCircle(b,c)},circleVsPolygon:function(a,b,c){return a.circleVsPolygon(b,c)},circleVsPoint:function(a,b,c){return a.circleVsPoint(b,c)},polygonVsAabb:function(a,
...
Grape2D.MedianCutBVHStrategy.prototype.solve=function(a){for(var b={left:[],right:[],endState:!1},c=Infinity,d=-Infinity,e=Infinity,f=-Infinity,g=0,k,h=0;h<a.length;h++)k=a[h].getBoundingBoxPosition(),c>k.getX()&&(c=k.getX()),d<k.getX()&&(d=k.getX()),e>k.getY()&&(e=k.getY()),f<k.getY()&&(f=k.getY());if(0===d-c&&0===f-e)return b.endState=!0,b;if(c+d>=e+f)for(g=(d+c)/2,h=0;h<a.length;h++)k=a[h].getBoundingBoxPosition(),k.getX()>g?b.right.push(a[h]):b.left.push(a[h]);else for(g=(f+e)/2,h=0;h<a.length;h++)k=
...
Grape2D.AabbBVFactory.prototype.createSceneBV=function(a,b){return new Grape2D.AABB({position:b.getLookAt(),width:a.getWidth()/b.getScale().getX(),height:a.getHeight()/b.getScale().getY()})};Grape2D.AabbBVFactory.prototype.merge=function(a,b){var c=Grape2D.Math.min(a.getMinX(),b.getMinX()),d=Grape2D.Math.max(a.getMaxX(),b.getMaxX()),e=Grape2D.Math.min(a.getMinY(),b.getMinY()),f=Grape2D.Math.max(a.getMaxY(),b.getMaxY());return new Grape2D.AABB({minX:c,minY:e,maxX:d,maxY:f})};
